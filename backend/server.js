...
app.post('/submit-form', (req, res) => {
  ...
  if (!formType) {
    return res.status(400).send({ result: false, message: 'Typ formularza jest wymagany' });
  }
  ...
  switch (formType) {
    ...
    case 'sponsor':
      ...
      break;
    default:
      return res.status(400).send({ result: false, message: 'nieznany typ formularza' });
  }
  res.status(200).send({ result: true, message: 'Dane formularza otrzymane poprawnie' });
});
